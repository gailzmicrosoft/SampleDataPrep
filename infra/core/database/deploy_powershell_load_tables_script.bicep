...
var myArguments = '-baseUrl "${baseUrl}" -resourceGroup "${resourceGroup().name}" -key_vault_name "${keyVaultName}" -postgres_server_name "${postgreSqlServerName}" -host_name "${postgresSqlServerFQN}" -database_name "${postgreSqlDbName}" -admin_principal_name "${adminPrincipalName}" -identity_name "${identityName}"'
...
var myPsUri = '${baseUrl}scripts/run_python_load_tables.ps1'
